...
layui.use(['form','element','laytpl'],function() {
    ...
    form.on('submit(relation)', function(data){
        ...
        var index=$('[name="index"]').val();
        var relation_data={"title":$('[name="relation[title]"]').val(),"page":$('[name="relation[page]"]').val(),"section":$('[name="relation[section]"]').val(),"url_rule":$('[name="relation[url_rule]"]').val(),"url_merge":$('[name="relation[url_merge]"]').val()}
        if(index){
    ...
    function add_field(type){
        if($('select[lay-filter="type"]').length > 0){
            ...
            laytpl($('#field').html()).render(data, function(html){
                ...
                var category_equivalents=$(html).find('[name="category_equivalents"]').val();
                category_equivalents=category_equivalents.split("#").join("\n");
                $('[name="category_equivalents"]').val(category_equivalents);
...
function relation_edit(){
    var data=JSON.parse(parent.$('[name="relation_url"]').val());
    var index=$('[name="index"]').val();
    layui.use('form', function(){
        var form = layui.form;
        form.val("relation", {
          "relation[title]": data[index]['title'],
          "relation[page]": data[index]['page'],
          "relation[section]": data[index]['section'],
          "relation[url_rule]": data[index]['url_rule'],
          "relation[url_merge]": data[index]['url_merge']
        });
    });
}